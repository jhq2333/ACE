...
### MANDATORY: SEMANTIC DEDUPLICATION & MERGE POLICY
MANDATORY - Before returning any `operations`, perform a semantic deduplication pass.

- Compute semantic similarity between the candidate content and existing bullets.
- Use a conservative merge threshold of **0.70** (70%). If you use embedding-based similarity, a threshold of **0.75** is recommended.

When similarity >= threshold, do NOT return an `ADD`. Instead return an `UPDATE` operation that merges the candidate into the canonical bullet. The `UPDATE` operation must include the following fields:
- `bullet_id`: id of the canonical bullet being updated
- `content`: the merged canonical phrasing (prefer concise, actionable wording)
- `justification`: short explanation of why the merge is safe
- `metadata`: aggregated counts (sum helpful/harmful) and `merged_from`: list of merged bullet ids

Merging policy (priority order):
1. Prefer the bullet with the highest `helpful` count as canonical.
2. If tied, prefer the shortest clear phrasing.
3. Preserve examples/evidence from merged bullets by appending an `examples` or `notes` subfield.

Example `UPDATE` operation the Curator should produce:
```
{
  "type": "UPDATE",
  ...
  "justification": "Merged duplicate recommendations about CSV loading; kept canonical phrasing",
  "metadata": {"helpful": 5, "harmful": 0},
  "merged_from": ["bullet_321", "bullet_418"]
}
```

If candidate overlap is partial (50-70%), the Curator should either:
- Return `UPDATE` with explicit `differences` and `scope` fields explaining what changed, or
- Return `ADD` but include a `deduplication_warning` advising a human reviewer.

MANDATORY: When no merge is performed, include an explicit `deduplication_check` entry indicating why the candidate is considered novel.
"""
