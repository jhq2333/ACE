...
class Playbook:
    ...
    def compact_prompt(self, max_bullets_per_section: int = 3, max_total: int = 30) -> str:
        """Produce a compact prompt-friendly string by selecting top bullets.

        Selection strategy:
        - For each section, sort bullets by (helpful - harmful) score descending.
        - Pick up to `max_bullets_per_section` bullets per section.
        - Stop once `max_total` bullets are collected across all sections.

        This helps reduce token count while keeping the most useful strategies.
        """
        parts: List[str] = []
        total = 0

        # iterate sections in stable order
        for section in sorted(self._sections.keys()):
            if total >= max_total:
                break
            ids = list(self._sections.get(section, []))
            # sort bullets by helpful-harmful score, then helpful count
            def score(bid: str):
                b = self._bullets[bid]
                return (b.helpful - b.harmful, b.helpful)

            ids.sort(key=score, reverse=True)
            picked = ids[:max_bullets_per_section]
            if not picked:
                continue
            parts.append(f"## {section}")
            for bid in picked:
                if total >= max_total:
                    break
                b = self._bullets[bid]
                # include id and content only (no counters) to save tokens
                parts.append(f"- {b.content}")
                total += 1

        return "\n".join(parts)

    def prompt_truncate(self, max_chars: int = 4000, **compact_kwargs) -> str:
        """Return a prompt string guaranteed to be <= max_chars.

        Attempts compact_prompt(**compact_kwargs) first and then truncates
        the resulting string to max_chars if necessary.
        """
        prompt = self.compact_prompt(**compact_kwargs)
        if len(prompt) <= max_chars:
            return prompt
        # naive truncation at word boundary
        truncated = prompt[: max_chars - 3]
        # try to avoid cutting in middle of a bullet
        last_newline = truncated.rfind("\n")
        if last_newline > max_chars // 4:
